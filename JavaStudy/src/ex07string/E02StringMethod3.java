...
public class E02StringMethod3 {
	...
	public static void main(String[] args) {
		 ...
		 * 		문자열의 뒤에서부터 찾는 것이 특징, 파일의 확장자를 찾아야하는
		 * 		업무에서는 뒤에서부터 찾아야 하는 경우 발생
		 */
		...
		System.out.println("lastIndexOf:"+
				str1.lastIndexOf("a")); // 14
		
		/*
		 * 12] replace : 특정 문자열을 찾아서 지정된 문자열로 변경
		 * 		찾는 문자열이 없어도 에러가 발생하진 않음
		 */
		...
		/*
		 * 13] split() : 문자열을 구분자를 통해 분리해서 String 배열로
		 * 		반환, 해당 구분자가 없는 경우 크기가 1인 배열로 반환
		 */
		System.out.println("### 13 ###");
		String phoneNumber = "010-1234-5678";
		// 전화번호를 (-)하이픈으로 분리하여 크기가 3인 배열로 반환
		String[] phoneArr = phoneNumber.split("-");
		// 배열의 크기만큼 반복하여 분리된 문자열 출력
		 ...
		 * 
		 * 방법 1 : 인덱스 하나만 사용 -> 사용한 인덱스부터 끝까지 잘라냄
		 * 방법 2 : 2개의 인덱스 사용 -> 시작, 종료 인덱스가 되므로 해당
		 * 						구간의 문자열을 잘라냄
		 */
