...
/*
 * foreach문(개선된for문)
 * 배열의 일부가 아니라 전체를 참조해야할 때 주로 사용
 * 배열의 요소를 참조의 형태로 가져와서 사용함
 * 따라서 참조의 목적으로만 사용해야하고 값은 변경 허용 안 됨
 * 형식]
 * 	for(자료형 참조변수 : 배열명){
 * 		실행문장;
 * 		참조변수를 통해 사용가능;
...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		 ...
		 * 아래 증가연산자를 통해 증가시킨 값은 실제 배열에 적용되지 않음
		 * for문 내에서만 증가된 값이 출력됨
		 */
		for(int b: arr) {
		...
		// 1증가는 기존 배열에 적용되지 않으므로 원본상태 그대로를 출력
		for(int c : arr) {
			System.out.print(c + " ");
		}
		System.out.println();
		...
		//2차원배열에서 하나의 행씩 접근해서 1차원배열을 꺼내옴
		for(int[] a : twoDim) {
